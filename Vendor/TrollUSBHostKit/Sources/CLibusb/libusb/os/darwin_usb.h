...
struct darwin_cached_device {
  ...
  bool                  in_reenumerate;
  int                   capture_count;
  /* iPadOS registers the same physical device under both IOUSBHostDevice and
     IOUSBDevice, so the removal iterator fires a detached notification twice.
     This flag makes darwin_devices_detached dereference the cached device only
     on the first notification; without it the second detach frees the cached
     device while an open handle still references it (use-after-free on close). */
  bool                  detach_seen;
