...
static enum libusb_error darwin_get_cached_device(struct libusb_context *ctx, io_service_t service, struct darwin_cached_device **cached_out,
                                                  UInt64 *old_session_id, int *was_cached);
...
static IOReturn usb_setup_device_iterator_for_class (io_iterator_t *deviceIterator, UInt32 location, const char *device_class) {
  CFMutableDictionaryRef matchingDict = IOServiceMatching(device_class);
...
/* iPadOS registers attached devices as IOUSBHostDevice while macOS keeps the
   legacy IOUSBDevice class. Scan both so enumeration works on either OS.
   (Local TrollScrcpy patch on UTM's utm-edition pin; upstream scans
   IOUSBDevice only.) */
static const char *darwin_scan_device_classes[] = {
  "IOUSBHostDevice",
  "IOUSBDevice",
};

/* Hotplug notifications mirror the initial scan's dual-class matching, so
   devices attaching after startup are caught on iPadOS too. */
#define DARWIN_HOTPLUG_CLASS_COUNT 2
static const char *darwin_hotplug_device_classes[DARWIN_HOTPLUG_CLASS_COUNT] = {
  "IOUSBHostDevice",
  "IOUSBDevice",
};
...
static void darwin_devices_attached (void *ptr, io_iterator_t add_devices) {
  ...
  while ((service = IOIteratorNext(add_devices))) {
    int was_cached = 0;
    ret = darwin_get_cached_device (NULL, service, &cached_device, &old_session_id, &was_cached);
    if (ret < 0 || !cached_device->can_enumerate) {
      continue;
    }

    /* Skip a cached entry that was already processed by the other IOService
       class match for the same physical device (not a re-enumeration). */
    if (was_cached && 0 == old_session_id) {
      usbi_dbg(NULL, "skipping duplicate class match for sessionID 0x%" PRIx64,
  ...
  while ((device = IOIteratorNext (rem_devices)) != 0) {
    ...
    list_for_each_entry(old_device, &darwin_cached_devices, list, struct darwin_cached_device) {
      if (old_device->session == session) {
        ...
        } else if (old_device->detach_seen) {
          /* the same physical device matched both IOUSBHostDevice and
             IOUSBDevice class removal notifications; only the first detach may
             dereference the cached device. */
          usbi_dbg (NULL, "skipping duplicate detach notification for sessionID: 0x%" PRIx64,
                    session);
        } else {
          old_device->detach_seen = true;
...
static void *darwin_event_thread_main (void *arg0) {
  ...
  io_iterator_t          libusb_rem_device_iterators[DARWIN_HOTPLUG_CLASS_COUNT];
  io_iterator_t          libusb_add_device_iterators[DARWIN_HOTPLUG_CLASS_COUNT];
  ...
  /* create notifications for removed and attached devices. like the initial
     scan (darwin_scan_devices) this matches both the legacy IOUSBDevice class
     and the IOUSBHostDevice class iPadOS uses, so devices attaching after the
     app has started are still caught. darwin_get_cached_device() dedupes by
     sessionID, so a device matched by both classes is only cached/processed
     once. */
  for (size_t c = 0; c < DARWIN_HOTPLUG_CLASS_COUNT; c++) {
    kresult = IOServiceAddMatchingNotification (libusb_notification_port, kIOTerminatedNotification,
                                                IOServiceMatching(darwin_hotplug_device_classes[c]),
                                                darwin_devices_detached,
                                                NULL, &libusb_rem_device_iterators[c]);
    ...
    kresult = IOServiceAddMatchingNotification(libusb_notification_port, kIOFirstMatchNotification,
                                               ...
                                               NULL, &libusb_add_device_iterators[c]);
  ...
  /* arm notifiers */
  for (size_t c = 0; c < DARWIN_HOTPLUG_CLASS_COUNT; c++) {
    darwin_clear_iterator (libusb_rem_device_iterators[c]);
    darwin_clear_iterator (libusb_add_device_iterators[c]);
  }
  ...
  /* delete iterators */
  for (size_t c = 0; c < DARWIN_HOTPLUG_CLASS_COUNT; c++) {
    IOObjectRelease (libusb_rem_device_iterators[c]);
    IOObjectRelease (libusb_add_device_iterators[c]);
  }
    ...
    list_for_each_entry(new_device, &darwin_cached_devices, list, struct darwin_cached_device) {
      ...
      if (new_device->session == sessionID) {
        ...
        *cached_out = new_device;
        /* remember that this cached entry already existed, so callers can skip
           re-processing it: on iPadOS the same physical device can be reported
           by both the IOUSBHostDevice and IOUSBDevice class matches. */
        *was_cached = 1;
        /* the device is attached again, so a future detach must deref */
        new_device->detach_seen = false;
        break;
      }
    }
...
static enum libusb_error darwin_scan_devices(struct libusb_context *ctx) {
  ...
  IOReturn kresult;
  int ret;
  size_t class_count = sizeof(darwin_scan_device_classes) / sizeof(darwin_scan_device_classes[0]);

  for (size_t c = 0; c < class_count; c++) {
    kresult = usb_setup_device_iterator_for_class (&deviceIterator, 0, darwin_scan_device_classes[c]);
    if (kresult != kIOReturnSuccess)
      continue;

    while ((service = IOIteratorNext (deviceIterator))) {
      int was_cached = 0;
      ret = darwin_get_cached_device (ctx, service, &cached_device, &old_session_id, &was_cached);
      if (ret < 0 || !cached_device->can_enumerate) {
        continue;
      }

      /* Skip a cached entry already processed by the other IOService class
         match for the same physical device (not a re-enumeration). */
      if (was_cached && 0 == old_session_id) {
        usbi_dbg(ctx, "skipping duplicate class match for sessionID 0x%" PRIx64,
                 cached_device->session);
        IOObjectRelease(service);
        continue;
      }
...
static int _darwin_detach_kernel_driver_locked (struct libusb_device_handle *dev_handle, uint8_t interface) {
  ...
  if (dpriv->capture_count == 0) {
    ...
    if (darwin_has_capture_entitlements ()) {
      ...
      if (kresult == kIOReturnBadMessageID) {
        /* iPadOS has no macOS-style console authorization agent for this
         * message. The capture operation still performs the kernel-side
         * com.apple.vm.device-access entitlement check, so continue only for
         * this specific unsupported authorization-message response. */
        usbi_info (ctx, "IOServiceAuthorize is unavailable on this service; trying entitlement-gated capture directly");
      } else if (kresult != kIOReturnSuccess) {
